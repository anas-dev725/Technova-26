...
for (const f of files) {
  if (fs.existsSync(path.join(process.cwd(), 'src/assets', f.file))) {
    ...
    const code = `export const ${f.name}Base64 = \n  ${chunks.join(' +\n  ')};\n`;
    fs.writeFileSync(path.join(process.cwd(), `src/assets/${f.name}Base64.ts`), code);
    console.log(`Generated src/assets/${f.name}Base64.ts`);
  }
}
